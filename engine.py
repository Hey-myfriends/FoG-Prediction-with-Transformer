...
def train_one_epoch(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in pbar:
        ...
        pbar.set_description("Train-> lr: {:.5f} loss: {:.4f} loss_ce: {:.4f}, loss_NW: {:.4f}, loss_preFoG: {:.4f}, loss_FoG: {:.4f}, class_error: {:.2f}%".format(
            meters["lr"].value, meters["loss"].avg, meters["loss_ce"].avg, meters["loss_NW"].avg, meters["loss_preFoG"].avg, meters["loss_FoG"].avg, meters["class_error"].avg
        ))
...
def evaluate(model: torch.nn.Module, criterion: torch.nn.Module,
    ...
    for samples, targets in pbar:
        ...
        loss_value = losses.cpu().item()
        meters["loss"].update(loss_value)
        pbar.set_description("Test-> loss: {:.4f} loss_ce: {:.4f}, loss_NW: {:.4f}, loss_preFoG: {:.4f}, loss_FoG: {:.4f}, class_error: {:.2f}%".format(
            meters["loss"].avg, meters["loss_ce"].avg, meters["loss_NW"].avg, meters["loss_preFoG"].avg, meters["loss_FoG"].avg, meters["class_error"].avg
        ))
    stats = {k: meter.global_avg for k, meter in meters.items()}
    print("Testing: ", stats, "\n\n")
    return stats

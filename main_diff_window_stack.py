...
class Arguments(object):
    def __init__(self, log_=True) -> None:
        ...
        self.lr_drop = 30
        self.gamma = 0.5
        self.tag = "with_diff_window_stacked"
...
def main():
    ...
    cls_weight, focal_scaler, aug = torch.ones(num_class), 2, False
    logger.info(f"in_chan: {in_chan}, d_model: {d_model}, num_class: {num_class}, cls_type: {cls_type}, aux_loss: {aux_loss}")

    for rpth in args.rootpath:
        logger.info("Current dataset folder is {}.".format(rpth))
        suffix = "_".join(rpth.split("_")[2:])
        all_samples = split_n_fold(n = args.numfolds, rootpth=rpth, level=args.level, seed=args.seed)
        samples_dist = {args.level: {fold: all_samples[fold] for fold in range(args.numfolds)}}
        with open(os.path.join(args.output_dir, f"{suffix}_samples.josn"), "w") as f:
            f.write(json.dumps(samples_dist, ensure_ascii=False, cls=JsonEncoder, indent=4, separators=(",", ":")))

        for fold in range(0, 1): #range(args.numfolds):
            logpath = os.path.join(args.output_dir, f"train_fold_{fold:02}-{suffix}.txt")
            ...
            # pdb.set_trace()
            dataset_train = build_dataset(train_samples[:], rootpth=rpth, level=args.level, mode="train", aug=aug)
            dataset_val = build_dataset(val_samples, rootpth=rpth, level=args.level, mode="val")
            ...
            logger.info("Fold {}-{}: Start training...".format(fold, suffix))
            ...
            for epoch in range(args.epochs):
                ...
                if args.output_dir:
                    if (epoch + 1) % args.lr_drop == 0 or (epoch + 1) % args.epochs == 0:
                        ckpt = os.path.join(args.output_dir, f"checkpoint_{epoch:04}_{fold:02}-{suffix}.pth")
                        torch.save({
                            "fold": fold,
                            "epoch": "{}-{}".format(epoch, suffix),
                            "args": args,
                        ...
                        ckpts.append(f"checkpoint_{epoch:04}_{fold:02}-{suffix}.pth")
                    with open(logpath, "a") as f:
                        f.write(json.dumps(log_stats) + "\n")

            plot_logs(args.output_dir, log_name=f"train_fold_{fold:02}-{suffix}.txt", fields=("loss", "loss_ce", "loss_NW", "loss_preFoG", "loss_FoG", "class_error"))
            total_time = time.time() - start_time
            total_time_str = str(datetime.timedelta(seconds=int(total_time)))
            logger.info('Fold.{}-{} Training time cost: {}'.format(fold, suffix, total_time_str))

            performance = test(model, data_loader_val, args.output_dir, args.device, level=args.level, ckpts=ckpts, n1=args.n1, n2=args.n2)
            with open(os.path.join(args.output_dir, f"test_fold_{fold:02}-{suffix}.json"), "w") as pf:
...
def test_ckpts():
    ...
    # pdb.set_trace()
    samples_split = None
    with open(os.path.join("./outputs_episode", "all_samples.josn"), "r") as f:

...
def output2(metrics_allfold: list):
    ...
    for m in metrics_allfold:
        ...
        PredMargin = [v[2] for v in m["episodeLevel_best"][0]["forcast_windows"].values() 
                if isinstance(v[2], int) and v[2] > 0]
...
# outputs_path = "../diff-settings/diff-signals/gyro/FoG-Prediction-with-Transformer/outputs_episode_gyro/"
# outputs_path = "./outputs_episode_SEC_ALSTM"
# outputs_path = "./outputs_episode_deepFoG"
outputs_path = "./outputs_episode_1_encoderlayer"
json_prefix = "test_fold*.json"
...
if len(json_all) == 0:
    json_all = glob.glob(os.path.join(outputs_path, "test*.json"))
# pdb.set_trace()
metrics_allfolds = []
